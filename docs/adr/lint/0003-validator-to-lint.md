...
lint 호출자는 입력 출처에 맞는 규칙 프로필을 명시할 수 있다. 프로필이 없으면 기존 생성
결과용 전체 규칙을 실행해 하위 호환성을 유지한다.

임포트 프로필은 캔버스 이탈, 텍스트 넘침, 비정상 크기처럼 원본 출처와 무관한 렌더 안전
규칙만 허용 목록으로 실행한다. 그리드·의미 계층, 생성용 최소 글자 크기, 요소 간 스타일
관계를 전제로 하는 규칙은 실행하지 않는다. 새 규칙은 import 프로필에 자동 포함되지 않으며
원본 충실도에 유효한지 검토한 뒤 명시적으로 추가한다.
...
- **출처별 신호 품질**: 임포트 결과에서 생성 계약 오탐을 제외하고 렌더 안전 결함에 집중

### Negative

- **자동 수정 없음**: 기존에는 제목 폰트가 자동으로 올라갔으나, 이제 사용자가 직접 수정 지시 필요
- **위반이 남을 수 있음**: 사용자가 lint 결과를 무시하면 위반이 그대로 렌더링됨
- **프로필 유지 비용**: 새 규칙마다 어떤 입력 출처에 적용할지 검토해야 함

## Acceptance Criteria

- 프로필을 지정하지 않은 기존 lint 호출은 전체 생성 규칙을 그대로 실행한다.
- import 프로필은 명시적 허용 목록만 실행하고 생성 전용 규칙을 보고하지 않는다.
- import 프로필도 감지와 리포트만 수행하며 디자인 스펙을 변경하지 않는다.
- 알 수 없는 프로필은 전체 규칙으로 조용히 폴백하지 않고 명확한 오류를 반환한다.
- 프로필 선택과 기본 호출 호환성을 검증하는 회귀 테스트가 존재한다.
