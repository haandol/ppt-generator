...
| 동일 행 폰트 일관성 | 같은 행의 동급 요소에서 대응 텍스트의 폰트 크기가 다름 | high 또는 medium |
| 동일 행 패딩 일관성 | 같은 행의 동급 요소에서 내부 패딩이 다름 | medium |
...
리뷰 프롬프트의 체크리스트 식별자와 응답 스키마의 식별자는 하나의 정식 집합으로
...
- 체크리스트와 응답 스키마의 변경은 함께 이뤄져야 하므로 계약 검증 유지 비용이
  추가된다.
