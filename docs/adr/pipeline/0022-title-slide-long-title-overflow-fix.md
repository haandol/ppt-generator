...
1. **validator의 height 강제 리셋**: `_fix_title_closing_main_position()`이 `_validate_textboxes()` 이후에 호출되면서, 텍스트 줄바꿈에 맞게 확장된 height를 다시 80px로 강제 리셋
...
- 관련 ADR: [0017-font-metric-text-overflow-prevention](./0017-font-metric-text-overflow-prevention.md)
