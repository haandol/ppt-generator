...
F4에서 생성한 HTML 슬라이드를 사용자의 자연어 수정 요청으로 반복 수정할 수 있어야 한다. 디자인 도구를 직접 다루지 않아도, "3번 슬라이드 제목 바꿔줘", "배경색을 파란색으로 바꿔줘" 같은 자연어 요청으로 결과물을 개선하는 인터랙티브 수정 루프를 지원한다.
...
- 이미지 재생성이 필요한 수정은 Titan Image API 추가 호출이 발생한다
