...
| **HTML 슬라이드** (F3) | 디자인 자유도 최대 | HTML/CSS로 슬라이드를 생성. 이미지는 필요한 경우에만 내부 생성. `position: absolute` 기반 자유 배치, 인라인 CSS로 제한 없는 스타일링. LLM의 코드 생성 능력을 최대한 활용 |
| **PPTX** (F5) | 편집 가능한 최종물 | HTML 요소를 PPTX 개별 객체(텍스트박스, 이미지, 도형)로 매핑. px→EMU 좌표 변환으로 위치·크기 보존. 디자인 자유도를 최대한 유지하면서 실무 편집성 확보 |
