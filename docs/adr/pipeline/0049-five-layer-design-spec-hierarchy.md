...
Date: 2026-05-26 (rolled up: ADR-0044/0045/0046 흡수; 데이터 무결성·lint 정책·autofit 기본값은 ADR-0053/0054/0055 로 분리)
...
- imported PPTX 는 design_doc=None 으로 들어감 (lazy backfill 은 ADR-0051).
...
- 5단 계층 *데이터 무결성* (PptxSlideSpec 재구성 필드 보존, element 부분 교체 시 비-design 메타 보존) — **ADR-0053**
- 5단 계층 *lint 정책* (단계적 lint, cross-layer link 검증, layout-tree severity 격상 등) — **ADR-0054**
- shape autofit 기본값 변경 — **ADR-0055**
- 다단 LLM 호출 분리 (비용/지연 영향 큼)
- imported PPTX lazy design_doc backfill — ADR-0051
...
- [ADR-0050: modify_component MCP 도구 — Section 단위 부분 수정](./0050-modify-component-mcp-tool.md)
...
- [ADR-0054: 5단 계층 lint 정책 — cross-layer 검증 + 단계적 실행](./0054-five-layer-lint-policy.md)
- [ADR-0055: PptxShape autofit 기본값 — shrink_text](./0055-shape-autofit-default-shrink-text.md)
