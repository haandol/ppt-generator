...
HTML 미리보기와 PPTX 내보내기는 같은 `DesignSpec`의 두 출력이다. 렌더 스키마,
HTML/PPTX 렌더러 또는 공용 텍스트·레이아웃 계산을 변경할 때 다음을 모두 지킨다.

1. 영향 받는 필드를 `shared`, `html_only`, `pptx_only`, `metadata` 중 하나로 렌더
   패리티 분류표에 등록한다. 새 필드가 미분류이면 테스트가 실패해야 한다.
2. `shared` 동작은 하나의 `DesignSpec`을 HTML과 PPTX로 각각 변환해 두 결과를 함께
   검증하는 교차 렌더 테스트를 추가하거나 갱신한다.
3. 출력 전용 필드는 브라우저 보안, 파일 패키징 등 구체적인 사유를 분류표에 기록하고
   해당 출력 테스트를 둔다.
4. 좌표·크기·줄바꿈·autofit·행간·문단 간격·불렛·선·화살표·효과 변경은 구조 테스트와
   별도로 실제 HTML/PPTX 렌더를 비교한다.

개별 렌더러 테스트만 수정하고 다른 출력의 영향 검토를 생략하면 완료로 간주하지 않는다.
...
- [ ] HTML/PPTX 공유 렌더 변경을 같은 `DesignSpec`으로 양쪽에서 검증했는가?
- [ ] 고위험 시각 변경이면 실제 HTML/PPTX 렌더를 비교했는가?
