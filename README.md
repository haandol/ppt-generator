...
발표 자료로 만들 내용을 `context.md` 등의 파일에 정리한 뒤, MCP 클라이언트에서 요청합니다:
...
디자인 스펙 생성 후 시각적 결함(줄바꿈, 겹침, 여백 불일치 등)을 자동 감지하고 수정할 수 있습니다. **자동으로 실행되지 않으며, 사용자가 명시적으로 요청해야 합니다.**
...
기존 PPTX 파일을 시스템에 가져와서 수정할 수 있습니다:

```
@presentation.pptx 를 import_pptx로 가져와줘.
```

임포트 후 `modify_design_spec`, `visual_qa`, `export_html`, `export_pptx` 등 모든 기존 도구를 사용할 수 있습니다. LLM 호출 없이 결정론적으로 파싱하므로 추가 비용이 발생하지 않습니다.
