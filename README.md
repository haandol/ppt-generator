...
MCP 서버 설정을 완료한 후, MCP 클라이언트(Claude Desktop, Claude Code 등)에서 자연어로 PPT 생성을 요청할 수 있습니다.

**1단계: PPT 생성 요청**

발표 자료로 만들 내용을 `context.md` 등의 파일에 정리한 뒤, 클라이언트에서 다음과 같이 요청합니다:
...
- **발표 목적** (`purpose`): 이 발표의 목적 (예: "사내 기술 공유", "고객 제안", "컨퍼런스 발표")
- **발표 시간** (`presentation_minutes`): 3~60분 (기본 15분). 슬라이드 수를 지정하지 않으면 발표 시간 기준으로 자동 계산됩니다.
- **청중 유형** (`audience_type`): `general`(일반), `technical`(기술), `executive`(의사결정자) 중 선택

이 정보를 바탕으로 아웃라인 → 스크립트 → 디자인 스펙 순서로 자동 생성됩니다. 각 단계에서 검토/수정 기회가 주어집니다.

**3단계: 파일 내보내기**

디자인 스펙 생성이 완료되면, PPTX 또는 HTML로 내보낼 수 있습니다:
