...
<slide_type_agenda>
아젠다 슬라이드 (slide_type: "content", component_hint: "agenda") 디자인 규칙:

- 프레젠테이션의 두 번째 슬라이드. 전체 발표 흐름/목차를 안내
- 레이아웃: 제목 텍스트박스 + 번호가 매겨진 항목 리스트 텍스트박스
  · 제목: left=40, top=40, width=1200, height=60
  · 기본 (1열 레이아웃): 제목 아래 단일 텍스트박스에 번호+항목을 세로로 나열 (left=40, top=120, width=1200, height=540)
  · 항목이 7개 이상으로 많을 경우에만 2열 레이아웃 허용 — 좌측(left=40, width=576) + 우측(left=648, width=576)
- 각 항목은 번호 + 제목 형태로, 간결하게 작성
