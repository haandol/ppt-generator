...
- two_column: 제목 + 좌우 2개 텍스트박스 (각 width 약 560px, gap 32px)
...
5. 요소 분리: 텍스트박스와 도형을 겹치지 않게 배치하세요.
   이유: 겹친 요소는 텍스트 가독성을 크게 저하시킵니다.

6. 여백 확보: 슬라이드 가장자리에서 최소 40px 여백을 유지하세요 (left 40 이상, top 40 이상, right 1240 이하, bottom 680 이하). 특히 하단 여백을 반드시 확보하세요: 모든 요소의 top_px + height_px는 680 이하. 콘텐츠가 슬라이드 하단에 딱 붙지 않도록 상단 제목 여백(40px)과 동일한 수준의 하단 여백을 유지하세요.
