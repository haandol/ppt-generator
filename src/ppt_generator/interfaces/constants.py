...
SLIDES_SYSTEM_PROMPT = (
    ...
    "- 22 (text_only): 제목 left=57 top=96 w=1152 h=56, 본문 left=64 top=180 w=1152. "
...
SLIDES_REGION_SYSTEM_PROMPT = (
    ...
    "\n"
    "높이 자동 조절 규칙 (반드시 준수 — 가장 중요한 규칙):\n"
    "- body 영역은 높이가 콘텐츠에 맞게 자동 조절됩니다. 고정 높이를 지정하지 마세요.\n"
    "- 절대 금지: height:100%, h-full, flex:1 등으로 컨테이너 높이를 부모에 맞춰 강제 확장하는 것.\n"
    "  · BAD:  <div class=\"flex flex-col h-full\"> → 부모 전체 높이까지 늘어남\n"
    "  · BAD:  <div class=\"three-col\" style=\"flex:1;\"> → 남은 공간 전부 차지\n"
    "  · GOOD: <div class=\"flex flex-col gap-3\"> → 콘텐츠에 맞게 자동 높이\n"
    "  · GOOD: <div class=\"three-col\"> → 콘텐츠에 맞게 자동 높이\n"
    "- 카드, 패널, 그리드 아이템은 콘텐츠에 맞는 자연스러운 높이를 가져야 합니다.\n"
    "- padding/gap/margin을 적절히 사용하세요: gap은 8~16px, padding은 12~16px.\n"
    "- 스크롤은 허용하지 않습니다.\n"
    "- 장식용 배경 요소(원, 도형 등)를 영역 내부에 사용하지 마세요.\n"
    "- 콘텐츠가 많으면 font-size를 줄이거나(최소 0.85rem) 항목 수를 줄이세요.\n\n"
    "디자인 원칙:\n"
    "- 폰트는 템플릿에서 전역 설정(Inter, Pretendard)되므로 별도 지정하지 마세요.\n"
    "- 모노스페이스 폰트가 필요하면 font-family:'Roboto Mono', monospace를 사용하세요.\n"
    "- 슬라이드 간 일관된 디자인 테마를 유지하세요.\n"
