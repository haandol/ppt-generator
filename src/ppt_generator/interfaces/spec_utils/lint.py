...
from ppt_generator.interfaces.spec_utils.lint_rules import ALL_RULES, RULES_BY_LAYER
...
def lint_slide_spec(
    ...
    if stop_on_layer_error:
        ...
        for layer in _LAYER_ORDER:
            ...
            _run_rules(RULES_BY_LAYER.get(layer, []), spec, slide_index, result)
        ...
        if layers:
            rules = [
                r for layer in target_layers for r in RULES_BY_LAYER.get(layer, [])
            ]
        else:
            rules = ALL_RULES
