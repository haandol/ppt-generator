...
- lint 결과는 `ppt-design` 스킬의 "lint 자동 처리" 기준으로 다룬다 — 경미한 warning 은
  visual QA 로 검증 후 실제 결함만 자가 수정하고, 중대한 건(error·overflow·레이아웃
  파손·메시지 변경)은 사용자에게 확인받는다. 모든 warning 을 매번 묻지도, 검증 없이
  넘기지도 않는다.
